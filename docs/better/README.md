...
| 06 | [Ambiente local](06-ambiente-local.md) | Três modos de rodar e o dataset de demonstração |
...
3. **A cascata de seleção da dotação usa `<select>` nativo em cinco níveis,
   sem busca.** Funciona na demonstração com 20 dotações; uma LOA municipal
   real tem milhares. É a tela crítica de conversão do produto.

## Estado verificado

Rodado localmente em 18/08/2026, no commit `2393ca8`:

| Verificação | Resultado |
| --- | --- |
| `npm install` | ok (Node 25.8, npm 11.11) |
| `npm run typecheck` | sem erros |
| `npm test` | 39 testes, 6 arquivos |
| `next build` | 35 rotas |
| `next dev` sem banco | todas as rotas em HTTP 200 (estados vazios) |

## Manutenção

Estes documentos descrevem o sistema **no commit `2393ca8`**. Ao mexer em
